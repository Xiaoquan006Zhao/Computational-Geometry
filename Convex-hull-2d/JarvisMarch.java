...
/**
 * Computing convex hull in 2d space by implementing Javis's March. The essence
 * of this algorithm is first sort the input by polar angle wrt. to a selected
 * point. Then update the reference and continue the sorting and selection.
 *
 * Input to constructor: ArrayList of points Output(ch): ArrayList of the convex
 * hull generated by the input points Time Complexity: O (nh). The run time is
 * output sensitive and h is the number of vertices in the convex hull. Space
 * Complexity: O (n).
 */
